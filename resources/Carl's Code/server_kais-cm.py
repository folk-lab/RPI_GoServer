...
def exit_handler():
    ...
    print "exiting"
    running = False
    print 'set running False, sending J, E'
    port.write("J") # in case user doesn't turn off
    sleep(0.01)
    port.write("E") # in case user doesn't turn off
    print 'joining port thread'
    sleep(0.01)
    port.flush()
    port_thread.join()
    port.close()
    print 'joining socket threads'
...
def port_listen_loop():
    global cap_prescale,cap_period,actual_frequency,actual_auxfrequency,unostatus,flags,overload_count,unlock_count,unlock_state
    while running:
        length = 0
        while (length == 0):
            header = port.read(1)
...
def socket_listen_loop(s):
    global num_stages, i_rc, q_rc, receiving_data,actual_frequency, actual_auxfrequency, amplitude, gain1, gain2, gain3, time_constant,phase,all_gain,flags,prescale,cap_freq
    while running:
        data = s.recv(1024)
...
if len(sys.argv) == 1:
    ...
    try:
        ...
        # version of pyserial is old
        print "(note: your version of pyserial is < 2.6)"
        import glob
        for port in glob.glob("/dev/ttyUSB*"):
            print port
            empty = False
    if empty:
        print "None - things work better when they are connected!"
    sys.exit(1)

port_name = sys.argv[1]
# ip, port may need to change
if len(sys.argv) > 3:
...
# enter loop, listening for client connections
while True:
    try:
        conn, addr = server_socket.accept()
